 ...
 */
export const B2_KEY_CAPABILITIES = {
	LIST_KEYS:   'listKeys',
...
class Key extends B2Object {
	...
	get expirationTimestamp() {
		if (!this._expirationTimestamp && this._validDurationInSeconds) {
			...
			now.setMilliseconds(now.getTime() + this._validDurationInSeconds);
	 ...
	 * 
	 * @returns {Promise<Key>} This object with the applicationKey property set.
	 */
	async create() {
		this.checkClient();
	 ...
	 * Deletes this key.
	 * 
	 * @returns {Promise<boolean>} True if this key was successfully deleted.
	 */
	async delete() {
		this.checkClient();
