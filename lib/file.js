...
class File extends B2Object {
	 ...
	 * 
	 * @returns {Promise<File>} This file if this file was successfully uploaded.
	 */
	async upload(args) {
		...
		if ((this.contentLength > recommendedPartSize || this.contentLength > SINGLE_FILE_MAX_SIZE) && this.contentLength <= LARGE_FILE_MAX_SIZE) {
		...
		if (this.contentLength > SINGLE_FILE_MIN_SIZE && this.contentLength <= SINGLE_FILE_MAX_SIZE) {
	...
	async uploadLarge(args = {}) {
		...
		if (this.contentLength < LARGE_FILE_MIN_SIZE || this.contentLength > LARGE_FILE_MAX_SIZE) {
			throw new InvalidFileSizeError;
		}
		...
		// @ts-ignore
		await this.data.on('readable', async () => {
			let chunk;
	 ...
	 * Hides this file.
	 * 
	 * @returns {Promise<boolean>} True if this file was successfully hidden.
	 */
	async hide() {
		this.checkClient();
	 ...
	 * 
	 * @returns {Promise<boolean>} True if this file was successfully deleted.
	 */
	async deleteVersion() {
		this.checkClient();
	 ...
	 * @returns {Promise<B2DownloadAuthorization>?}
	 */
	...
	async getDownloadUrl(args = {}) {
		...
		if (
			...
			const bucket = await this._b2.bucket.get({
	...
	async download(args = {}) {
		...
		return this._b2.file.downloadById({
			...args,
			fileId: this.fileId,
		});
	}
