...
import { createSHA1Checksum, getContentLength, makeB2SSEHeaders, rangeArrayToString } from '../utils';
...
class LargeFileActions extends B2Action {
	 ...
	 * @param {(progressEvent: any) => void}    [args.onUploadProgress]     A callback to pass to Axios `onUploadProgress`.
	 * @returns {Promise<AxiosResponse>}
	 * @throws {InvalidArgumentError} When the arguments do not pass validation.
	 */
	...
	async listAllParts(args) {
		...
		while (startPartNumber !== null) {
			const response = await this.listParts({...args, startPartNumber: startPartNumber});
	 ...
	 * @b2TransactionClass C
	 * @param {object}       args                                   Method arguments.
	 * @param {string}       args.sourceFileId                      The ID of the source file being copied.
	 * @param {string}       args.largeFileId                       The ID of the large file the part will belong to.
	 * @param {string}       args.partNumber                        A number from 1 to 10000. The parts uploaded for one file must have contiguous numbers, starting with 1.
	 * @param {string|Array} [args.range]                           The range of bytes to copy. If not provided, the whole source file will be copied.
	 * @param {B2SSEConfig}  [args.sourceServerSideEncryption]      If present, specifies the parameters for Backblaze B2 to use for accessing the source file data using Server-Side Encryption.
	 * @param {B2SSEConfig}  [args.destinationServerSideEncryption] If present, specifies the parameters for Backblaze B2 to use for encrypting the copied data before storing the destination file using Server-Side Encryption.
	 * 
	 * @throws {InvalidArgumentError} When the arguments do not pass validation.
	 * 
	 * @returns {Promise<AxiosResponse>}
	...
	async copyPart(args) {
		...
		return await this._b2.client.post('/b2_copy_part', {
			sourceFileId:                    args.sourceFileId,
			largeFileId:                     args.largeFileId,
			partNumber:                      args.partNumber,
			range:                           rangeArrayToString(args.range),
	 ...
	 * 
	 * See {@link https://www.backblaze.com/b2/docs/b2_list_unfinished_large_files.html}.
	 * 
	 * @b2TransactionClass C
	 * @param {object} args                    Method arguments.
	 * @param {string} args.bucketId           The bucket to look for file names in.
	 * @param {number} [args.startFileId]      The first upload to return.
	 * @param {string} [args.namePrefix]       Only files whose names match the prefix will be returned.
	 * @param {number} [args.maxFileCount=100] Maximum number of files to return.
	 * 
	 * @throws {InvalidArgumentError} When the arguments do not pass validation.
	 * 
	 * @returns {Promise<AxiosResponse>}
	 */
	async listUnfinished(args) {
		if (!args || !args.bucketId) {
			throw new InvalidArgumentError('The `bucketId` parameter is required');
		}

		return await this._b2.client.post('/b2_list_unfinished_large_files', {
			bucketId:        args.bucketId,
			namePrefix:      args.namePrefix,
			startPartNumber: args.startFileId,
	...
	/**
	 * Lists all large file uploads that have been started, but have not been finished or canceled. 
	 * 
	 * @see {B2.listUnfinished}
	 * 
	 * @b2TransactionClass C
	 * @param {object} args                    Method arguments.
	 * @param {string} args.bucketId           The bucket to look for file names in.
	 ...
	 * @returns {Promise<Array<object>>}
	 */
	async listAllUnfinished(args) {
		const files = [];
		let startFileId = args.startFileId;

		while (startFileId !== null) {
			const response = await this.listUnfinished({...args, startFileId: startFileId});
