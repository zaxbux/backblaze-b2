...
class FileActions extends B2Action {
	...
	async upload(args) {
		...
		if (!args.uploadUrl && args.bucketId) {
			const response = await this.b2.bucket.getUploadUrl({
		...
		return await this.b2.client.post(args.uploadUrl, args.data, {
			headers: {
				...bzInfoHeaders,
	...
	async getDownloadAuthorization(args) {
		...
		return await this.b2.client.post('/b2_get_download_authorization', {
			...
			b2ContentDisposition:   args.b2ContentDisposition || null,
			b2ContentLanguage:      args.b2ContentLanguage || null,
			b2Expires:              args.b2Expires || null,
			b2CacheControl:         args.b2CacheControl || null,
			b2ContentEncoding:      args.b2ContentEncoding || null,
			b2ContentType:          args.b2ContentType || null,
		});
	}
	 ...
	 * @param {string}      [args.axios.headersOnly=false]  Specifies if a HEAD request should be done instead of a GET request.
	 * @param {string}      [args.axios.responseType]       Axios responsType.
	...
	async downloadByName(args = {}) {
		return await this.b2.client.request({
			url:                this.getDownloadURL(args),
			method:             args.axios.headersOnly === true ? 'head' : 'get',
			responseType:       args.axios.responseType || null,
			encoding:           null,
			transformResponse:  args.axios.transformResponse || null,
			onDownloadProgress: args.axios.onDownloadProgress || null,
			headers: {
	...
	async getDownloadURL(args) {
		...
		const params = new URLSearchParams({
			...
			b2ContentEncoding:    args.b2ContentEncoding || null,
			b2ContentType:        args.b2ContentType || null,
			serverSideEncryption: JSON.stringify(args.serverSideEncryption || null),
		});

		return new URL(`${this.b2.authorization.downloadUrl}/file/${args.bucketName}/${fileName}` + params);
	}
	...
	async downloadById(args) {
		...
		let url = `${this.b2.authorization.downloadUrl}/b2api/${API_VERSION}/b2_download_file_by_id`;
		...
		let params = {
			...
			b2ContentEncoding:    args.b2ContentEncoding || null,
			b2ContentType:        args.b2ContentType || null,
			serverSideEncryption: args.method.toLowerCase() === 'get' ? JSON.stringify(args.serverSideEncryption || null) : null,
		};
		...
		return await this.b2.client.request({
			...
			headers: {
				...
				...(args.method.toLowerCase() === 'post' ? makeB2SSEHeaders(args.serverSideEncryption) : {}),
			},
			...
			data: (args.method.toLowerCase() || 'post') === 'post' ? params : null,
		});
	}
	...
	async copy(args) {
		return await this.b2.client.post('/b2_copy_file', {
			...
			fileName:                        args.fileName,
			destinationBucketId:             args.destinationBucketId || null,
			range:                           args.range || null,
			contentType:                     args.contentType || null,
			fileInfo:                        args.fileInfo || null,
			sourceServerSideEncryption:      args.sourceServerSideEncryption || null,
			destinationServerSideEncryption: args.destinationServerSideEncryption || null,
		});
	}
}
