...
import { BadAuthTokenError, ExpiredAuthTokenError, InvalidArgumentError } from '../errors';
import { createSHA1Checksum, encodeB2String, getContentLength, makeB2SSEHeaders, makeBzInfoHeaders, rangeArrayToString } from '../utils';
...
/**
 * @typedef {import('axios').AxiosTransformer} AxiosTransformer
 */
...
class FileActions extends B2Action {
	...
	async upload(args) {
		...
		try {
			return await this._b2.client.post(args.uploadUrl, args.data, {
				...
				// @ts-ignore
				_retry: true, // Don't try to get an account authorization token
			});
		} catch (error) {
			// Fetch a new upload URL and auth token on failure
			if (error instanceof BadAuthTokenError || error instanceof ExpiredAuthTokenError) {
				const authData = await (await this._b2.bucket.getUploadUrl({ bucketId: args.bucketId })).data;
	...
	async listAllNames(args) {
		...
		while (startFileName !== null) {
			const response = await this.listNames({...args, startFileName});
	...
	async listAllVersions(args) {
		...
		while (startFileName !== null && startFileId !== null) {
			const response = await this.listVersions({...args, startFileName, startFileId});
	 ...
	 * @param {object}      [args.axios]                    Axios options.
	 ...
	 * @param {string}          args.fileName                   Name of the file to download.
	 * @param {string}          [args.authorization]            An account auth token to access all files in a private bucket, or a download auth token to access files with that prefix.
	 * @param {string|number[]} [args.range]                    The range of bytes to download.
	 * @param {string}          [args.b2ContentDisposition]     B2 will use it as the value of the 'Content-Disposition' header, overriding the uploaded value.
	 * @param {string}          [args.b2ContentLanguage]        B2 will use it as the value of the 'Content-Language' header, overriding the uploaded value.
	 * @param {string}          [args.b2Expires]                B2 will use it as the value of the 'Expires' header, overriding the uploaded value.
	 * @param {string}          [args.b2CacheControl]           B2 will use it as the value of the 'Cache-Control' header, overriding the uploaded value.
	 ...
	 * @param {object}          [args.axios]                    Axios options.
	 * @param {boolean}         [args.axios.headersOnly=false]  Specifies if a HEAD request should be done instead of a GET request.
	 * @param {ResponseType}    [args.axios.responseType]       Axios responseType.
	 * @param {AxiosTransformer}        [args.axios.transformResponse]  Axios transformResponse.
	 * @param {(progressEvent: any) => void}        [args.axios.onDownloadProgress] Axios onDownloadProgress (browser only).
	 * @returns {Promise<AxiosResponse>}
	 * @throws {InvalidArgumentError} When the arguments do not pass validation.
	 */
	async downloadByName(args) {
		...
		return await this._b2.client.request({
			url:                this.getDownloadURL(args).toString(),
			...
			headers: {
				Range: args.range ? `bytes=${rangeArrayToString(args.range)}` : undefined,
				...(args.serverSideEncryption ? makeB2SSEHeaders(args.serverSideEncryption) : {})
			},
		});
	}
	...
	getDownloadURL(args) {
		...
		return new URL(`${this._b2.authorization.downloadUrl}/file/${args.bucketName}/${fileName}` + (params.length > 0 ? '?' : '') + urlParams);
	}
	 ...
	 * @param {string}          [args.b2ContentType]            B2 will use it as the value of the 'Content-Type' header, overriding the uploaded value.
	 * @param {B2SSEConfig}     [args.serverSideEncryption]     Required if the files were uploaded with server-side encryption using customer-managed keys.
	 * @param {Method}          [args.method=post]              Request method to use, either `head`, `get`, or `post`.
	 * @param {object}          [args.axios]                    Axios options.
	 * @param {boolean}         [args.axios.headersOnly=false]  Specifies if a HEAD request should be done instead of a GET request.
	 * @param {ResponseType}          [args.axios.responseType]       Axios responseType.
	 * @param {AxiosTransformer}        [args.axios.transformResponse]  Axios transformResponse.
	...
	async downloadById(args) {
		...
		return await this._b2.client.request({
			...
			headers: {
				...
				Range:         args.range ? `bytes=${rangeArrayToString(args.range)}` : undefined,
				...(args.serverSideEncryption && args.method === 'post' ? makeB2SSEHeaders(args.serverSideEncryption) : {}),
			},
			responseType: args.axios.responseType,
			transformResponse: args.axios.transformResponse,
			onDownloadProgress: args.axios.onDownloadProgress,
	 ...
	 * @param {string} args.fileName Name of the file.
	 * @returns {Promise<number>}             The number of versions deleted.
	 * @throws {InvalidArgumentError} When the arguments do not pass validation.
	 */
	 ...
	 * @param {string|number[]} [args.range]                           Range of bytes to copy. If not provided, the whole source file will be copied.
