...
class BucketActions extends B2Action {
	...
	async create(args) {
		...
		if (!args || !args.bucketType) {
			throw new InvalidArgumentError('The `bucketType` parameter is required.');
		}

		return await this.b2.client.post('/b2_create_bucket', {
	...
	async delete(args) {
		...
		return await this.b2.client.post('/b2_delete_bucket', {
			accountId: this.b2.authorization.accountId,
