...
/**
 * @typedef {import('../bucket').B2BucketType} B2BucketType
 */

/**
 * @typedef {import('../bucket').CorsRule} CorsRule
 */

/**
 * @typedef {import('../bucket').LifecycleRule} LifecycleRule
 */
...
class BucketActions extends B2Action {
	 ...
	 * @param {B2BucketType}           [args.bucketType=allPrivate]       Type of bucket to create.
	 * @param {Object<string, string>} [args.bucketInfo]                  Optional metadata to store with the bucket.
	 * @param {CorsRule[]}             [args.corsRules]                   Optional CORS rules.
	 * @param {LifecycleRule[]}        [args.lifecycleRules]              Optional B2 lifecycle rules.
	 * @param {B2SSEConfig}            [args.defaultServerSideEncryption] Optional server-side encryption configuration.
	 ...
	 * @param {B2BucketType[]} [args.bucketTypes] Filter for bucket types.
	 ...
	 * 
	 * @throws {InvalidArgumentError} When the arguments do not pass validation.
	 * @throws {BucketNotFoundError}  When the bucket with the provided ID cannot be found.
	 * 
	...
	async get(args) {
		...
		throw new BucketNotFoundError(`The bucket '${args.bucketId || args.bucketName}' does not exist`);
	}
