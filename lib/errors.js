/**
 * @file Defines custom error classes.
 * @module Errors
 */

import { HTTP_STATUS_CODES } from './constants';
import { urlPathnameEndsWith } from './utils';
 ...
 * @property {number} data.status  The numeric HTTP response code.
 * @property {string} data.code    A single-identifier code that identifies the error.
 * @property {string} data.message A human-readable message, in English, saying what went wrong.
 */

/**
 * @classdesc Base class for B2 API SDK errors.
 */
...
/**
 * @classdesc The size of the file to be uploaded was outside of the minimum/maximum size allowed by Backblaze.
 * @augments B2Error
 */
export class InvalidFileSizeError extends B2Error {}

/**
 * @classdesc Bucket does not exist.
 * @augments B2Error
 */
export class BucketNotFoundError extends B2Error {}

/**
 * @augments B2RequestError
 */
export class BadRequestError extends B2RequestError {}

/**
 * @augments BadRequestError
 */
...
/**
 * @classdesc File does not exist.
 * @augments NotFoundError
 */
...
export async function handleResponseError(error, b2) {
	...
	if (error.response.status === HTTP_STATUS_CODES.UNAUTHORIZED) {
		...
		if (urlPathnameEndsWith(originalRequest.url, '/b2_authorize_account')) {
		...
		if (!originalRequest._retry && (
			...
			originalRequest.headers.Authorization = b2.authorization.authorizationToken;
	...
	if (error.response.status === HTTP_STATUS_CODES.NOT_FOUND) {
		...
		if (urlPathnameEndsWith(originalRequest.url, fileEndpoints)) {
