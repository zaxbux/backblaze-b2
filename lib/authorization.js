...
class B2Authorization {
	...
	/**
	 * @returns {{ bucketId: string, bucketName: string }?} The bucket that this application key is restricted to, if any.
	 */
	getBucketRestriction() {
		if (this.allowed.bucketId && this.allowed.bucketName) {
			return {
				bucketId:   this.allowed.bucketId,
				bucketName: this.allowed.bucketName,
			};
		}
	}
